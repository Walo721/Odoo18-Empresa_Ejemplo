# Odoo 18 - Empresa Ejemplo 🚀

![Odoo](https://img.shields.io/badge/Odoo-18.0-714B67?logo=odoo&logoColor=white)
![License](https://img.shields.io/badge/License-LGPL--3-blue.svg)

Este repositorio contiene la configuración, personalización y módulos personalizados de
un entorno Doodba para la implementación de Odoo 18 en una empresa de ejemplo. El
objetivo es servir como base para la gestión de procesos empresariales como ventas,
compras, inventario y facturación.

## 📋 Tabla de Contenidos

- [Características](#-características)
- [Requisitos Previos](#-requisitos-previos)
- [Instalación & guía](#-instalación-&-guía)

## ✨ Características

- **Versión:** Odoo 18 con PostgreSQL 16
- **Módulos OCA:**
  - l10n_es
  - mis_builder
  - web_responsive (opcional)

## 💻 Requisitos Previos

Antes de instalar, asegúrate de tener:

- Python 3.10+
- Dependencias de Python listadas en `dependencies/pip.txt`
- Docker

## 🛠️ Instalación & guía

This project is a Doodba scaffolding. Check upstream docs on the matter:

- [General Doodba docs](https://github.com/Tecnativa/doodba).
- [Doodba copier template docs](https://github.com/Tecnativa/doodba-copier-template)
- [Doodba QA docs](https://github.com/Tecnativa/doodba-qa)

[![Doodba deployment](https://img.shields.io/badge/deployment-doodba-informational)](https://github.com/Tecnativa/doodba)
[![Last template update](https://img.shields.io/badge/last%20template%20update-v9.1.1-informational)](https://github.com/Tecnativa/doodba-copier-template/tree/v9.1.1)
[![Odoo](https://img.shields.io/badge/odoo-v18.0-a3478a)](https://github.com/odoo/odoo/tree/18.0)
[![LGPL-3.0-or-later license](https://img.shields.io/badge/license-LGPL--3.0--or--later-success})](LICENSE)
[![pre-commit](https://img.shields.io/badge/pre--commit-enabled-brightgreen?logo=pre-commit&logoColor=white)](https://pre-commit.com/)
